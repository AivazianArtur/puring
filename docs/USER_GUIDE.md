...
**See more examples in [docs/examples](https://github.com/AivazianArtur/aio_uring/tree/main/docs/examples)**
...
```
### Initialize `AioUringLoop`
To start using `aio_uring` user need to initialize special `io_uring` loop - `AioUringLoop` \
Its quite simple:
```python
...
#### `File` operations
`aio_uring.File` object acts as interface to true asynchronous file operations in Python.
**See full [File API](docs/API.md)**

You can import it by
```python
from aio_uring import File
``` 
Let's create simple example of writing and reading from file:

1. Open file asynchronously
```python
file = await aio_uring.open_file(path='path/to/file.txt')
``` 
2. Write to file, asynchronously. Better use bytes in v0.4.0
```python
await file.write(data=b'Hello aio_uring!\n')
```
...
async def file_simple_example():
    file = await aio_uring.open_file(path=TEMPFILE)
    await file.write(data=b'Hello, aio_uring!\n')
    await file.read()
    await file.close()
```

**Important note** - you can await this function only with new `AioUringLoop` loop.
...
asyncio.run(file_simple_example(), loop_factory=aio_uring.AioUringLoop)
```
...
async def file_simple_example():
    async with await aio_uring.open_file(path=TEMPFILE) as file:
        await file.write(data=b'Hello, aio_uring!\n')
        await file.read()
...
#### `Socket` operations
`aio_uring.Socket` object acts as interface to asynchronous socket operations in Python using `io_uring`, not `epoll`.
**See full [Socket API](docs/API.md)**

You can import it by
```python
from aio_uring import Socket
```
...
1. Prepare socket
```python
socket = await aio_uring.prep_socket()
```
2. Bind to socket
```python
...
As with file operations, socket operations must be executed using the `AioUringLoop`:

```python
import aio_uring

async def socket_simple_example():
    server_socket = await aio_uring.prep_socket()
    await server_socket.bind('127.0.0.1', 12878)
    await server_socket.listen()

    client_socket = await aio_uring.prep_socket()
...
asyncio.run(socket_simple_example(), loop_factory=aio_uring.AioUringLoop)
```
...
async def socket_simple_example():
    async with await aio_uring.prep_socket() as server_socket:
        await server_socket.bind('127.0.0.1', 12878)
        await server_socket.listen()

        async with await aio_uring.prep_socket() as client_socket:
            await client_socket.connect('127.0.0.1', 12878)
...
#### TransferMode
To use operations in zero-copy mode, use `AioUringLoop.transfer_mode` context manager, with `aio_uring.TRANSFER_MODE` Enum:
```python
async def transfer_mode():
    loop = asyncio.get_running_loop()
    with loop.transfer_mode(mode=aio_uring.TRANSFER_MODE.ZERO_COPY):
...
#### StreamStrategy
To use Multishot operations, use `stream_strategy` context manager, with `aio_uring.STREAM_STRATEGY` Enum. In v0.4.0 its better to not use it: 
```python
async def stream_strategy():
    loop = asyncio.get_running_loop()
    with loop.stream_strategy(stream=aio_uring.STREAM_STRATEGY.MULTISHOT):
        await simple_socket_example()

asyncio.run(stream_strategy(), loop_factory=aio_uring.AioUringLoop)
```
There are `ONESHOT` and `MULTISHOT` values in `STREAM_STRATEGY` Enum

#### BufferMode
To use operations in zero-copy mode, use `AioUringLoop.buffer_mode` context manager with `aio_uring.BUFFER_MODE` Enum:
##### Fixed buffer mode
...
asyncio.run(buffer_mode(), loop_factory=aio_uring.AioUringLoop)
```

There are `NORMAL`, `FIXED`, `PROVIDED` and `BUF_RING` values in `BUFFER_MODE` Enum

#### ExecutionContext
To set context manager with different types of all regimes above - use `AioUringLoop.execution_context` context manager:
```python
