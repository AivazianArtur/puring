...
def next_port():
    ...
    # when PORT_URINGIO was reused across n=100/500/2000.
...
# ---------------------------------------------------------------------
# 4. uringio: server + all n clients on the single io_uring ring, no
#    worker threads and no epoll either.
# ---------------------------------------------------------------------

async def uringio_c10k(n, port):
    print(f'Running uringio sockets (io_uring), n={n}')
...
def run():
    ...
    with asyncio.Runner(loop_factory=uringio.UringioLoop) as runner:
        for n in conn_counts:
            run_step('uringio', lambda n, p: runner.run(uringio_c10k(n, p)), n, next_port())
