...
# ---------------------------------------------------------------------
# 4. puring: server + all n clients on the single io_uring ring, every
#    send/recv is its own SQE, no worker threads, no epoll.
# ---------------------------------------------------------------------

async def puring_fanout(n, port):
    print(f'Running puring sockets fanout (io_uring), n={n}')
...
def run():
    ...
    with asyncio.Runner(loop_factory=puring.PuringLoop) as runner:
        for n in conn_counts:
            run_step('puring', lambda n, p: runner.run(puring_fanout(n, p)), n, next_port())
