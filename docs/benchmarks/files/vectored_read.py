...
def drop_cache(path=SRC):
    ...
    the same way whether the next reader is os.readv, os.pread, or uringio.
...
# ---------------------------------------------------------------------
# 5. uringio readv: one IORING_OP_READV submission per iteration,
#    scattering into NR_CHUNKS buffers - no worker threads at all.
# ---------------------------------------------------------------------

async def uringio_readv():
    print('Running uringio readv (single SQE per iteration)')
    f = await uringio.open_file(path=SRC)
    buffers = [bytearray(CHUNK_SIZE) for _ in range(NR_CHUNKS)]
...
# ---------------------------------------------------------------------
# 6. uringio scalar: NR_CHUNKS separate read() SQEs per iteration,
#    submitted concurrently and awaited together.
# ---------------------------------------------------------------------

async def uringio_scalar_read_concurrent():
    print('Running uringio scalar read, concurrent per iteration')
    f = await uringio.open_file(path=SRC)
...
# ---------------------------------------------------------------------
# 7. uringio readv with FIXED (pre-registered) buffers - skips the
#    per-call buffer mapping/pinning that the default mode pays for.
# ---------------------------------------------------------------------

async def uringio_readv_fixed():
    print('Running uringio readv FIXED buffers')
    f = await uringio.open_file(path=SRC)
    buffers = [bytearray(CHUNK_SIZE) for _ in range(NR_CHUNKS)]
...
def run_suite(cold):
    ...
    with asyncio.Runner(loop_factory=uringio.UringioLoop) as runner:
        timed_async('uringio readv', uringio_readv, runner)
        timed_async('uringio scalar concurrent', uringio_scalar_read_concurrent, runner)
        # timed_async('uringio readv FIXED', uringio_readv_fixed, runner)
