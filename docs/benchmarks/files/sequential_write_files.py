...
FILE_URINGIO_SEQ = FILES_FOLDER + 'uringio_seq.bin'
FILE_URINGIO_SEQ_FIXED = FILES_FOLDER + 'uringio_seq_fixed.bin'
FILE_URINGIO_SEQ__INIT = FILES_FOLDER + 'uringio_seq__init.bin'
...
async def uringio_write_sequential():
    print('Running io_uring sequential write')
    uring_file = await uringio.open_file(path=FILE_URINGIO_SEQ)
    start = time.perf_counter()
    for _ in range(ITERATIONS):
...
async def uringio_write_sequential__fixed():
    print('Running io_uring sequential write in FIXED mode')
    uring_file = await uringio.open_file(path=FILE_URINGIO_SEQ_FIXED)
    buf = bytearray(DATA)
...
def run():
    ...
    with asyncio.Runner(loop_factory=uringio.UringioLoop) as runner:
        t = runner.run(uringio_write_sequential())
        results.append(('uringio_seq', t))
    with asyncio.Runner(loop_factory=uringio.UringioLoop) as runner:
        t = runner.run(uringio_write_sequential__fixed())
        results.append(('uringio_seq FIXED', t))
