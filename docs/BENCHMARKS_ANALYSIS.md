...
- **Cons**
    ...
    - **Socket Closure Issues(v0.4.1):** Prone to `OSError 98` (Address already in use) under heavy loads; socket closing logic requires optimization.
    - **Missing Read Optimizations(v0.4.1):** Cold cache reads and random read workloads currently lag behind standard methods and need optimizations like `O_DIRECT` support.
