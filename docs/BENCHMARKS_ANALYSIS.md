...
##### Benchmark essence
This benchmark evaluates the performance of sequentially writing large amounts of data to disk in a Python environment on Linux. aio_uring here is the only backend that offers native async operations
##### Compared methods
...
- aio_uring, sequential writing with own implementation of write
- aio_uring, same thing in Fixed mode

#### Write files using vector operations
![vectored write](assets/benchmark_results/files/vectored_write.png)
##### Results
Another benchmark that shows strong part of aio_uring - native async file operations is faster that vectored operations in concurrently.
##### Benchmark essence
...
- aio_uring, own implementation of writev
- aio_uring, sequential writing
- aio_uring, concurrent writing
- aio_uring, concurrent writing in Fixed mode
...
However, when data is not inside cache aio_uring is still a bit slower. I think there is a lot of rooms for optimizations to be faster, for example O_DIRECT support
##### Benchmark essence
...
- aio_uring vectored read
- aio_uring vectored read in FIXED mode
- aio_uring scalar concurrent read

#### Open/close operation on little files in different depth
![little files](assets/benchmark_results/files/little_files.png)
##### Results
This benchmark shows quite interesting result - aio_uring is fastest in context of async writing, but standard synchronous write is much faster. \
...
- aio_uring, the only true asynchronous python backed for file I/O  
...
This benchmark confirms 2 important points from benchmarks above:
  ...
  - There are rooms for optimizations of `aio_uring`, because it is quite surprising that it is not the fastest asyncio backend, while it is the only backend with native asyncio support    
##### Benchmark essence
...
##### Results
This benchmark shows how aio_uring, by its io_uring nature, is optimized for scaling with the lowest latency and highest speed. \
...
- aio_uring, concurrent socket handling. Based on io_uring
...
- But with increase of connections aio_uring is faster than others, and its performance keeps almost on constant level

But also there is a thing that chart above dont show - we need some optimization for fastest socket closing, because in a lot of cases there were OSError98. See the [issue](https://github.com/AivazianArtur/aio_uring/issues/26)
##### Benchmark essence
