...
/*
 * AioUringFile/AioUringSocket both start with PyObject_HEAD, and registry.c only
 * ever touches them through Py_INCREF/Py_DECREF (it never dereferences
 * aio_uring-specific fields). Any real PyObject can stand in for them here.
 */
static PyObject *
make_future(void) {
    return PyList_New(0);
...
test_registry_add__increfs_file_and_socket_when_present(void) {
    ...
    int idx = registry_add(
        reg, future, NULL, ONESHOT, 1, (AioUringFile *)fake_file, (AioUringSocket *)fake_socket, NULL, NULL
    );
    TEST_ASSERT(idx >= 0, "registry_add should succeed");
