...
test_open_file__creates_new_file_with_default_flags(void) {
    ...
    char path[] = "/tmp/puring_ctest_open_XXXXXX";
...
test_open_file__explicit_rdonly_on_existing_file(void) {
    ...
    const char *content = "hello puring";
...
test_open_file__nonexistent_without_creat_fails(void) {
    ...
    const char *path = "/tmp/puring_ctest_does_not_exist_xyz";
    unlink(path);
...
test_open_file__O_RDONLY_dont_collides_with_sentinel(void) {
    ...
    int ret = open_file(&ring, 1, AT_FDCWD, "/tmp/puring_ctest_rdonly_no_collision_xyz", O_RDONLY, 0, 0644, NO_TIMEOUT);
...
static void
test_puring_read__success_reads_full_content(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    char buf[16] = {0};
    int ret = puring_read(&ring, 1, fd, buf, sizeof(buf), 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_read submit should succeed");
...
static void
test_puring_read__respects_offset(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    char buf[16] = {0};
    int ret = puring_read(&ring, 1, fd, buf, sizeof(buf), 5, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_read submit should succeed");
...
static void
test_puring_read__zero_size_reads_nothing(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    char buf[4] = {0};
    int ret = puring_read(&ring, 1, fd, buf, 0, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_read submit should succeed even with size 0");
...
static void
test_puring_read__invalid_fd_returns_ebadf(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");

    char buf[16] = {0};
    int ret = puring_read(&ring, 1, 9999 /* not a valid fd */, buf, sizeof(buf), 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
static void
test_puring_read__sqe_unavailable_returns_minus_one(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    char buf[16];
    int ret = puring_read(&ring, 1, 0, buf, sizeof(buf), 0, NO_TIMEOUT);
    TEST_ASSERT(ret == -1, "puring_read should fail fast when SQ is exhausted");
...
static void
test_puring_readv__success_across_multiple_buffers(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_readv(&ring, 1, fd, iov, 3, 0, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_readv submit should succeed");
...
static void
test_puring_readv__short_read_on_small_file(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_readv(&ring, 1, fd, iov, 2, 0, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_readv submit should succeed");
...
static void
test_puring_readv__zero_vecs(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_readv(&ring, 1, fd, NULL, 0, 0, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_readv submit should succeed even with 0 vecs");
...
static void
test_puring_readv__invalid_fd_returns_ebadf(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_readv(&ring, 1, 9999, &iov, 1, 0, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
static void
test_puring_write__success_writes_content(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_write(&ring, 1, fd, data, (unsigned)strlen(data), 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_write submit should succeed");
...
static void
test_puring_write__respects_offset(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    char data[] = "YYY";
    int ret = puring_write(&ring, 1, fd, data, 3, 5, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_write submit should succeed");
...
static void
test_puring_write__invalid_fd_returns_ebadf(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");

    char data[] = "x";
    int ret = puring_write(&ring, 1, 9999, data, 1, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
static void
test_puring_writev__success_from_multiple_buffers(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_writev(&ring, 1, fd, iov, 2, 0, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_writev submit should succeed");
...
static void
test_puring_writev__invalid_fd_returns_ebadf(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_writev(&ring, 1, 9999, &iov, 1, 0, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
static void
test_puring_close_file__success_closes_fd(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_close_file(&ring, 1, fd, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_close_file submit should succeed");
...
static void
test_puring_close_file__invalid_fd_returns_ebadf(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");

    int ret = puring_close_file(&ring, 1, 9999, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
/* ---------- puring_fsync / puring_fdatasync ---------- */

static void
test_puring_fsync__success_on_regular_file(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_fsync(&ring, 1, fd, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_fsync submit should succeed");
...
static void
test_puring_fdatasync__success_on_regular_file(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_fdatasync(&ring, 1, fd, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_fdatasync submit should succeed");
...
static void
test_puring_fsync__invalid_fd_returns_ebadf(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");

    int ret = puring_fsync(&ring, 1, 9999, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
static void
test_puring_splice__file_to_pipe_to_file(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    /* file -> pipe */
    int ret1 = puring_splice(&ring, 1, src_fd, 0, pipefd[1], -1, (int)len, 0, NO_TIMEOUT);
    ...
    /* pipe -> file */
    int ret2 = puring_splice(&ring, 2, pipefd[0], -1, dst_fd, 0, (int)len, 0, NO_TIMEOUT);
...
static void
test_puring_splice__invalid_fd_returns_error(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_splice(&ring, 1, 9999, 0, pipefd[1], -1, 4, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
static void
test_puring_read_buffer_select__success(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_read_buffer_select(&ring, 1, fd, (unsigned)strlen(content), 0, bgid, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_read_buffer_select submit should succeed");
...
static void
test_puring_read_buffer_select__invalid_fd_returns_ebadf(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_read_buffer_select(&ring, 1, 9999, sizeof(pool), 0, bgid, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
static void
test_puring_readv_buffer_select__success(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_readv_buffer_select(&ring, 1, fd, (unsigned)strlen(content), 0, bgid, 0, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_readv_buffer_select submit should succeed");
...
/* ---------- puring_read_fixed / puring_write_fixed ---------- */

static void
test_puring_write_fixed__then_read_fixed_roundtrip(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int wret = puring_write_fixed(&ring, 1, fd, write_buf, len, 0, /*buf_index=*/0, NO_TIMEOUT);
    TEST_ASSERT(wret == 1, "puring_write_fixed submit should succeed");
    int wres = wait_one_cqe(&ring);
    TEST_ASSERT(wres == (int)len, "write_fixed should write exactly len bytes");

    int rret = puring_read_fixed(&ring, 2, fd, read_buf, len, 0, /*buf_index=*/1, NO_TIMEOUT);
    TEST_ASSERT(rret == 1, "puring_read_fixed submit should succeed");
...
static void
test_puring_read_fixed__invalid_buf_index_returns_efault(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_read_fixed(&ring, 1, fd, buf, 4, 0, 5, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
static void
test_puring_read_multishot__success_on_socket(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_read_multishot(&ring, 1, sv[0], -1, bgid, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "puring_read_multishot submit should succeed");
...
static void
test_puring_read_multishot__regular_file_unsupported(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = puring_read_multishot(&ring, 1, fd, 0, bgid, NO_TIMEOUT);
    TEST_ASSERT(ret == 1, "submit itself should still succeed");
...
main(void) {
    ...
    RUN_TEST(test_puring_read__success_reads_full_content);
    RUN_TEST(test_puring_read__respects_offset);
    RUN_TEST(test_puring_read__zero_size_reads_nothing);
    RUN_TEST(test_puring_read__invalid_fd_returns_ebadf);
    RUN_TEST(test_puring_read__sqe_unavailable_returns_minus_one);

    RUN_TEST(test_puring_readv__success_across_multiple_buffers);
    RUN_TEST(test_puring_readv__short_read_on_small_file);
    RUN_TEST(test_puring_readv__zero_vecs);
    RUN_TEST(test_puring_readv__invalid_fd_returns_ebadf);

    RUN_TEST(test_puring_write__success_writes_content);
    RUN_TEST(test_puring_write__respects_offset);
    RUN_TEST(test_puring_write__invalid_fd_returns_ebadf);

    RUN_TEST(test_puring_writev__success_from_multiple_buffers);
    RUN_TEST(test_puring_writev__invalid_fd_returns_ebadf);

    RUN_TEST(test_puring_close_file__success_closes_fd);
    RUN_TEST(test_puring_close_file__invalid_fd_returns_ebadf);

    RUN_TEST(test_puring_fsync__success_on_regular_file);
    RUN_TEST(test_puring_fdatasync__success_on_regular_file);
    RUN_TEST(test_puring_fsync__invalid_fd_returns_ebadf);

    RUN_TEST(test_puring_splice__file_to_pipe_to_file);
    RUN_TEST(test_puring_splice__invalid_fd_returns_error);

    RUN_TEST(test_puring_read_buffer_select__success);
    RUN_TEST(test_puring_read_buffer_select__invalid_fd_returns_ebadf);
    RUN_TEST(test_puring_readv_buffer_select__success);

    RUN_TEST(test_puring_write_fixed__then_read_fixed_roundtrip);
    RUN_TEST(test_puring_read_fixed__invalid_buf_index_returns_efault);

    RUN_TEST(test_puring_read_multishot__success_on_socket);
    RUN_TEST(test_puring_read_multishot__regular_file_unsupported);
