...
test_bind_listen_connect_accept__full_flow(void) {
    ...
    int bret = aio_uring_bind(&ring, 1, listen_fd, (struct sockaddr *)&addr, sizeof(addr), NO_TO);
    TEST_ASSERT(bret == 1, "aio_uring_bind submit should succeed");
    ...
    int lret = aio_uring_listen(&ring, 2, listen_fd, 1, NO_TO);
    TEST_ASSERT(lret == 1, "aio_uring_listen submit should succeed");
    int lres = wait_one_cqe(&ring);
    TEST_ASSERT(lres == 0, "listen should succeed");

    struct sockaddr_storage peer = {0};
    socklen_t peerlen = sizeof(peer);
    int aret = aio_uring_accept(&ring, 3, listen_fd, (struct sockaddr *)&peer, &peerlen, 0, NO_TO);
    TEST_ASSERT(aret == 1, "aio_uring_accept submit should succeed");

    int client_fd = socket(AF_INET, SOCK_STREAM, 0);
    TEST_ASSERT(client_fd >= 0, "posix socket() for client failed");

    int cret = aio_uring_connect(&ring, 4, client_fd, (struct sockaddr *)&addr, sizeof(addr), NO_TO);
    TEST_ASSERT(cret == 1, "aio_uring_connect submit should succeed");
...
static void
test_aio_uring_connect__refused_on_closed_port(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = aio_uring_connect(&ring, 1, client_fd, (struct sockaddr *)&addr, sizeof(addr), NO_TO);
...
static void
test_aio_uring_send_recv__roundtrip(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int sret = aio_uring_send(&ring, 1, cfd, msg, strlen(msg), 0, NO_TO);
    TEST_ASSERT(sret == 1, "aio_uring_send submit should succeed");
    int sres = wait_one_cqe(&ring);
    TEST_ASSERT(sres == (int)strlen(msg), "send should report full length");

    char buf[64] = {0};
    int rret = aio_uring_recv(&ring, 2, sfd, buf, sizeof(buf), 0, NO_TO);
    TEST_ASSERT(rret == 1, "aio_uring_recv submit should succeed");
...
static void
test_aio_uring_recv__peer_closed_returns_zero(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    char buf[16];
    int ret = aio_uring_recv(&ring, 1, sfd, buf, sizeof(buf), 0, NO_TO);
...
static void
test_aio_uring_sendto_recvfrom__roundtrip(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int rret = aio_uring_recvfrom(
        &ring, 2, server_fd, buf, sizeof(buf), (struct sockaddr *)&peer, sizeof(peer), 0, msg_hdr, NO_TO
    );
    TEST_ASSERT(rret == 1, "aio_uring_recvfrom submit should succeed");
    int rres = wait_one_cqe(&ring);
...
static void
test_aio_uring_sendmsg_recvmsg__roundtrip_multiple_iovecs(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int sret = aio_uring_sendmsg(&ring, 1, cfd, send_iov, 2, NULL, 0, 0, NO_TO);
    TEST_ASSERT(sret == 1, "aio_uring_sendmsg submit should succeed");
    int sres = wait_one_cqe(&ring);
    ...
    int rret = aio_uring_recvmsg(&ring, 2, sfd, recv_iov, 2, 0, NO_TO);
    TEST_ASSERT(rret == 1, "aio_uring_recvmsg submit should succeed");
    int rres = wait_one_cqe(&ring);
...
static void
test_aio_uring_recv_fixed__success(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = aio_uring_recv_fixed(&ring, 1, sfd, recv_buf, strlen(msg), 0, /*buf_index=*/0, NO_TO);
    TEST_ASSERT(ret == 1, "aio_uring_recv_fixed submit should succeed");
    int res = wait_one_cqe(&ring);
...
static void
test_aio_uring_recv_buffer_select__success(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = aio_uring_recv_buffer_select(&ring, 1, sfd, sizeof(pool), bgid, 0, NO_TO);
    TEST_ASSERT(ret == 1, "aio_uring_recv_buffer_select submit should succeed");
...
static void
test_aio_uring_accept_multishot__accepts_two_connections(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    struct sockaddr_storage peer = {0};
    socklen_t peerlen = sizeof(peer);
    int ret = aio_uring_accept_multishot(&ring, 1, listen_fd, (struct sockaddr *)&peer, &peerlen, 0, NO_TO);
    TEST_ASSERT(ret == 1, "aio_uring_accept_multishot submit should succeed");
...
static void
test_aio_uring_recv_multishot__receives_two_messages(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = aio_uring_recv_multishot(&ring, 1, sfd, /*len=*/0, bgid, 0, NO_TO);
    TEST_ASSERT(ret == 1, "aio_uring_recv_multishot submit should succeed");
...
static void
test_aio_uring_send_zc__two_completions(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = aio_uring_send_zc(&ring, 1, cfd, msg, strlen(msg), 0, NO_TO);
    TEST_ASSERT(ret == 1, "aio_uring_send_zc submit should succeed");
...
static void
test_aio_uring_close_socket__success(void) {
    struct io_uring ring;
    TEST_ASSERT(ring_init(&ring) == 0, "ring_init failed");
    ...
    int ret = aio_uring_close_socket(&ring, 1, fd, NO_TO);
    TEST_ASSERT(ret == 1, "aio_uring_close_socket submit should succeed");
    int res = wait_one_cqe(&ring);
...
main(void) {
    ...
    RUN_TEST(test_aio_uring_connect__refused_on_closed_port);

    RUN_TEST(test_aio_uring_send_recv__roundtrip);
    RUN_TEST(test_aio_uring_recv__peer_closed_returns_zero);

    RUN_TEST(test_aio_uring_sendto_recvfrom__roundtrip);
    RUN_TEST(test_aio_uring_sendmsg_recvmsg__roundtrip_multiple_iovecs);

    // RUN_TEST(test_aio_uring_recv_fixed__success);
    RUN_TEST(test_aio_uring_recv_buffer_select__success);

    RUN_TEST(test_aio_uring_accept_multishot__accepts_two_connections);
    RUN_TEST(test_aio_uring_recv_multishot__receives_two_messages);

    RUN_TEST(test_aio_uring_send_zc__two_completions);

    RUN_TEST(test_aio_uring_close_socket__success);
