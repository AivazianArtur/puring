...
async def test_buffer_mode__nested_fixed_raises_and_outer_still_valid(tmp_path):
    ...
    with loop.buffer_mode(
        mode=puring.BUFFER_MODE.FIXED, payload_type=puring.PAYLOAD_TYPE.IOVEC, buffers=[outer_buf]
    ):
        with pytest.raises(expected_exception=RuntimeWarning):
            with loop.buffer_mode(
                mode=puring.BUFFER_MODE.FIXED, payload_type=puring.PAYLOAD_TYPE.IOVEC, buffers=[inner_buf]
            ):
                pass
